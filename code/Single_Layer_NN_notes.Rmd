...
We will denote the summation (our objective function) $\C (\theta)$.
...
Our goal is to find $\arg \min_{\theta} \C (\theta)$ with gradient descent:

1. Start with a guess $\theta^0$ for all parameters in $\theta$, and set $t = 0$
2. Iterate until $\C (\theta)$ fails to decrease:
    * $\theta^{t + 1} \leftarrow \theta^t - \rho * \nabla\C (\theta)$
    
$\rho$ is our learning rate: it controls how quickly we respond to the gradient. $\nabla\C (\theta)$ points in the direction of the greatest increase, so we subtract it to move in the direction of the greatest decrease. Our change in parameter values is proportional to both the learning rate and the gradient magnitude.
...
* let $\err = \hat y_i - y_i$
    * so $\err$ is our *i*th residual
    * note that $\err = \left(\beta_0 + \sum_{k = 1}^K \beta_k * g(z_{ik})\right) - y_i$
...
$$
\begin{aligned}
\C (\theta) &= \sum_{i = 1}^n \frac{1}{2} \left[y_i - \left(\beta_0 + \sum_{k = 1}^K \beta_k * g(w_{k0} + \sum_{j = 1}^p w_{kj} x_{ij})\right)\right]^2 \\ \\
...
&= \sum_{i = 1}^n \frac{1}{2} \left[\err\right]^2
\end{aligned}
$$
...
$$
\begin{aligned}
\frac{\partial \C}{\partial \beta_0} &= \frac{\partial}{\partial \beta_0} \sum_{i = 1}^n \frac{1}{2} \left[\left(\beta_0 + \sum_{k = 1}^K \beta_k * g(z_{ik})\right) - y_i\right]^2 \\ \\
...
$$
\begin{aligned}
\frac{\partial \C}{\partial \beta_k} &= \frac{\partial}{\partial \beta_k} \sum_{i = 1}^n \frac{1}{2} \left[\left(\beta_0 + \sum_{k = 1}^K \beta_k * g(z_{ik})\right) - y_i\right]^2 \\ \\
...
$$
\begin{aligned}
\frac{\partial \C}{\partial w_{k0}} &= \frac{\partial}{\partial w_{k0}} \sum_{i = 1}^n \frac{1}{2} \left[\left(\beta_0 + \sum_{k = 1}^K \beta_k * g(z_{ik})\right) - y_i\right]^2 \\ \\
...
$$
\begin{aligned}
\frac{\partial \C}{\partial w_{kj}} &= \frac{\partial}{\partial w_{kj}} \sum_{i = 1}^n \frac{1}{2} \left[\left(\beta_0 + \sum_{k = 1}^K \beta_k * g(z_{ik})\right) - y_i\right]^2 \\ \\
...
&= \sum_{i = 1}^n \err \ \beta_k \ g'(z_{ik}) \ x_{ij}
\end{aligned}
$$
...
$$\C (\theta) = \sum_{i = 1}^n \frac{1}{2} \left[\err\right]^2$$
...
\nabla \C (\theta) = 
\begin{bmatrix}
\frac{\partial \C}{\partial w_{1, 0}}  \\
\frac{\partial \C}{\partial w_{2, 0}} \\
\vdots \\
\frac{\partial \C}{\partial w_{1, 1}} \\
\vdots \\
\frac{\partial \C}{\partial w_{K, p}} \\
\frac{\partial \C}{\partial \beta_{0}} \\
\frac{\partial \C}{\partial \beta_{1}} \\
\vdots \\
\frac{\partial \C}{\partial \beta_{K}}
\end{bmatrix} = \sum_{i = 1}^n 
\begin{bmatrix}
\err \ \beta_1 \ g'(z_{i1})  \\
\err \ \beta_2 \ g'(z_{i2}) \\
\vdots \\
\err \ \beta_1 \ g'(z_{i1}) \ x_{i1} \\
\vdots \\
\err \ \beta_K \ g'(z_{ik}) \ x_{ip} \\
\err \\
\err \ g(z_{i1}) \\
\vdots \\
\err \ g(z_{ik})
