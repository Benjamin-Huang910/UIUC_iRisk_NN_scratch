...
#install.packages(c("tidyverse", "keras", "MASS", "logistf", "Metrics", "prediction", "ggplot2", "plotly"))
library(tidyverse)
library(keras)
library(MASS)
library(logistf)
...
# Initialize lists to store confusion matrices for each digit
conf_matrix_train_list <- list()
conf_matrix_test_list <- list()
...
for (digit in 0:9) {
  ...
  # Calculate confusion matrix for the current digit
  confusion_matrix_train <- table(Actual = y_train_digit, Predicted = pred_train)
  confusion_matrix_test <- table(Actual = y_test_digit, Predicted = pred_test)
  ...
  conf_matrix_train_list[[as.character(digit)]] <- confusion_matrix_train
  conf_matrix_test_list[[as.character(digit)]] <- confusion_matrix_test
}
```
...
for (digit in 0:9) {
  ...
  cat(" Confusion Matrix (Training Set):\n", confusion_matrix_train_list[[as.character(digit)]], "\n")
  cat(" Confusion Matrix (Test Set):\n", confusion_matrix_test_list[[as.character(digit)]], "\n")
  cat("\n")
}
...
plot_mnist <- function(plt, prediction) {
  ## create image
  image(x = 1:28,
        y = 1:28,
  ...
  text(x = 15, y = 25, labels = paste("Prediction:", prediction), col = "red", cex = 1.5)
}

# Display the first 36 predictions from pred_test_list for each digit
par(mfcol = c(6, 6))
par(mar = c(0, 0, 0, 0))

for (digit in 0:9) {
  cat("Digit", digit, " Predictions:", pred_test_list[[as.character(digit)]][1:36], "\n")
  for (i in 1:36) {
    plot_mnist(x_test[i, , ], pred_test_list[[as.character(digit)]][i])
  }
}
